...
-(void)getGoogleObjectsWithQuery:(NSString *)query andMapRegion:(MKCoordinateRegion)region andNumberOfResults:(int)numResults addressesOnly:(BOOL)addressOnly andReferer:(NSString *)referer;
{
    ...
    NSString *testString = [[NSString alloc] initWithFormat:@"http://ajax.googleapis.com/ajax/services/search/local?v=1.0&mrt=localonly&q=%@&rsz=%@&start=0&sll=%f,%f&sspn=%f,%f",query,numberOfResults,centerLat,centerLng,spanLat,spanLng];
...
- (void)connection:(NSURLConnection *)conn didReceiveResponse:(NSURLResponse *)response {
 //   NSLog(@"didReceiveResponse in Google Location Connection, response:%@",response);
	[responseData setLength:0];
}


- (void)connection:(NSURLConnection *)conn didFailWithError:(NSError *)error {
    NSLog(@"didFailWithError in GoogleLocalConnection");
...
- (void)connectionDidFinishLoading:(NSURLConnection *)conn 
{
    ...
    // NSLog(@"parsedJSON in GoogleLocalConnection:%@",parsedJSON);
	...
	if ([jsonError code]==0) {
			...
			if ([responseStatus isEqualToString:@"200"]) {
                ...
                NSLog(@"results count for gResponseData in GoogleLocalConnection:%d",[[gResponseData objectForKey:@"results"] count]);
    ...
    //            NSLog(@"GoogleLocalObjects before:%@",googleLocalObjects);
    ...
    //            NSLog(@"GoogleLocalObjects after:%@",googleLocalObjects);
